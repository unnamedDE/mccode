...
module.exports = (args, options = {fullError: false, generateJSON: false, noErrors: false, noOutput: false, noWhitelines: false, noCredits: false}, callback = () => {}) => {
  const {fullError, generateJSON, noErrors, noOutput, noWhitelines, noCredits} = options;
  ...
  function modPath(p, changeDir) {
    ...
    if(targetIndex) return dir.splice(targetIndex).splice(1).reverse().join(path.sep) + path.sep + changeDir + path.sep + dir
    return p
  }
